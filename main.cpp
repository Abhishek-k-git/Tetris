...
int main() {
	...
	RenderWindow window(VideoMode(board.getScreenWidth(), board.getScreenHeight()), "Created by Abhishek");
	window.setFramerateLimit(60);

	board.createNewTetriminous();
	...
	while (window.isOpen()) {
		...
		if (live) {
			...
			// key up
			if (Keyboard::isKeyPressed(Keyboard::Up) && !up) board.RotatePiece(), up = 1;
